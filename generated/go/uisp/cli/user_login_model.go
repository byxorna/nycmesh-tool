...
func registerUserLoginLastNewsSeenDate(depth int, cmdPrefix string, cmd *cobra.Command) error {
	...
	lastNewsSeenDateDescription := `Required. Last news check time in ISO format.`

	var lastNewsSeenDateFlagName string
	if cmdPrefix == "" {
		lastNewsSeenDateFlagName = "lastNewsSeenDate"
	} else {
		lastNewsSeenDateFlagName = fmt.Sprintf("%v.lastNewsSeenDate", cmdPrefix)
	}

	_ = cmd.PersistentFlags().String(lastNewsSeenDateFlagName, "", lastNewsSeenDateDescription)
...
func registerUserLoginUpdated(depth int, cmdPrefix string, cmd *cobra.Command) error {
	...
	updatedDescription := `Required. Last time the user was updated.`
...
func retrieveUserLoginLastNewsSeenDateFlags(depth int, m *models.UserLogin, cmdPrefix string, cmd *cobra.Command) (error, bool) {
	...
	if cmd.Flags().Changed(lastNewsSeenDateFlagName) {
		...
		lastNewsSeenDateFlagValueStr, err := cmd.Flags().GetString(lastNewsSeenDateFlagName)
		if err != nil {
			return err, false
		}
		var lastNewsSeenDateFlagValue strfmt.DateTime
		if err := lastNewsSeenDateFlagValue.UnmarshalText([]byte(lastNewsSeenDateFlagValueStr)); err != nil {
			return err, false
		}
		m.LastNewsSeenDate = &lastNewsSeenDateFlagValue
