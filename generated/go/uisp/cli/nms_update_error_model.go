...
func registerNmsUpdateErrorTime(depth int, cmdPrefix string, cmd *cobra.Command) error {
	...
	timeDescription := `Required. Time when the error occurred.`
	...
	_ = cmd.PersistentFlags().String(timeFlagName, "", timeDescription)
...
func retrieveNmsUpdateErrorTimeFlags(depth int, m *models.NmsUpdateError, cmdPrefix string, cmd *cobra.Command) (error, bool) {
	...
	if cmd.Flags().Changed(timeFlagName) {
		...
		timeFlagValueStr, err := cmd.Flags().GetString(timeFlagName)
		if err != nil {
			return err, false
		}
		var timeFlagValue strfmt.DateTime
		if err := timeFlagValue.UnmarshalText([]byte(timeFlagValueStr)); err != nil {
			return err, false
		}
