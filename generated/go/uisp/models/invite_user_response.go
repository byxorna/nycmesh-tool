// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// InviteUserResponse invite user response
//
// swagger:model InviteUserResponse
type InviteUserResponse struct {

	// invitation link
	// Required: true
	InvitationLink *string `json:"invitationLink"`

	// message
	// Required: true
	Message *string `json:"message"`

	// result
	// Required: true
	Result *bool `json:"result"`
}

// Validate validates this invite user response
func (m *InviteUserResponse) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateInvitationLink(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateMessage(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateResult(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *InviteUserResponse) validateInvitationLink(formats strfmt.Registry) error {

	if err := validate.Required("invitationLink", "body", m.InvitationLink); err != nil {
		return err
	}

	return nil
}

func (m *InviteUserResponse) validateMessage(formats strfmt.Registry) error {

	if err := validate.Required("message", "body", m.Message); err != nil {
		return err
	}

	return nil
}

func (m *InviteUserResponse) validateResult(formats strfmt.Registry) error {

	if err := validate.Required("result", "body", m.Result); err != nil {
		return err
	}

	return nil
}

// ContextValidate validates this invite user response based on context it is used
func (m *InviteUserResponse) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *InviteUserResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *InviteUserResponse) UnmarshalBinary(b []byte) error {
	var res InviteUserResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
